...
def select_img_des_kp(img_md5):
    con = sqlite3.connect(db_path, detect_types=sqlite3.PARSE_DECLTYPES)
    cur = con.cursor()
    cur.execute("select des,kp from img_sift where img_md5=?", (img_md5,))
    data = cur.fetchall()
    return data[0]
...
def select_row_md_and_des_all():
    s = time.time()
    con = sqlite3.connect(db_path, detect_types=sqlite3.PARSE_DECLTYPES)
    cur = con.cursor()
    cur.execute("select img_md5,des from img_sift")
    data = cur.fetchall()
    e = time.time()
    # print(data)
    # print(data[0])
    # print(len(data))
    print("select_row_md_and_des_all time:%s s" % (e - s))
    return data
...
def select_img_md5(path_md5):
    con = sqlite3.connect(db_path, detect_types=sqlite3.PARSE_DECLTYPES)
    cur = con.cursor()
    cur.execute("select img_md5 from img_path_md5 where path_md5=?",
                (path_md5,))
    img_md5 = cur.fetchall()
    return img_md5[0]


def delete_img_tag():
    con = sqlite3.connect(db_path, detect_types=sqlite3.PARSE_DECLTYPES)
    con.execute("DROP TABLE img_tag")
    con.commit()


def delete_img_path(path):
    con = sqlite3.connect(db_path, detect_types=sqlite3.PARSE_DECLTYPES)
    con.execute("delete from img_path_md5 where path=?", (path,))
    con.commit()
    print("ccccccccc")


def select_img_path_img_md5(img_md5):
    con = sqlite3.connect(db_path, detect_types=sqlite3.PARSE_DECLTYPES)
    cur = con.cursor()
    cur.execute("select COUNT(*) from img_path_md5 where img_md5=?",
                (img_md5,))
    data = cur.fetchall()[0][0]
    return data


def delete_img_sift(img_md5):
    con = sqlite3.connect(db_path, detect_types=sqlite3.PARSE_DECLTYPES)
    con.execute("delete from img_sift where img_md5=?", (img_md5,))
    con.commit()
    print("delete_img_sift")


def delete_img_tag_(img_md5):
    con = sqlite3.connect(db_path, detect_types=sqlite3.PARSE_DECLTYPES)
    con.execute("delete from img_tag where img_md5=?", (img_md5,))
    con.commit()
    print("delete_img_tag")

# select_img_path_img_md5("9e80a67c24ed362f052b3ece59c6a7d3")
