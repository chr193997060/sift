...
from Bow.bow_achieve import search_img_calss, com_db_img_class
from approximateShow.approximate_windows import ApproximateWindow
...
class Operating_Ui(object):
    ...
    def com_sitf(self):
        ...
        if self.img == None:
            ...
            self.img_md5, self.sift_img_list[
                self.img], self.sift_img_des, self.sift_img_kp, self.sift_img, _ = sift_img_com(self.img)
            e = time.time()
            print(e - s)
    ...
    def find_same_img(self):
        ...
        #     des1 = imagesift[2]
        #     img_md5 = imagesift[0]
        #     kp1 = pickle.loads(imagesift[3])
        # else:

        img_md5, _, des1, kp1, _, img = sift_img_com(self.img)

        img = cv2.imdecode(np.fromfile(self.img, dtype=np.uint8), cv2.IMREAD_COLOR)

        com_db_img_class()  # 对数据库里的img添加分类tag到数据库

        s = time.time()
        pathss = search_img_calss(img_md5, des1, kp1, img)
        # print(pathss)
        pathss.sort(key=lambda x: x[1], reverse=True)
        e = time.time()
        print(e - s)
        print("-" * 30)
        ...
        # self.Traverse_img_path()
        # if len(self.imgs_name) > 50:
        #     com_approximate_img_path(4, des1, self.imgs_name)
        # else:
        ...
        # print(pathss)
        if pathss is None:
            msg_box = QMessageBox(QMessageBox.Information, '提示', '未找到相似图像！！')
            msg_box.exec_()
        elif len(pathss) == 0:
            msg_box = QMessageBox(QMessageBox.Information, '提示', '未找到相似图像！！')
            msg_box.exec_()
        else:
            msg_box = QMessageBox(QMessageBox.Information, '提示', '已检索到相似图像！')
            msg_box.exec_()
            self.win = QtWidgets.QMainWindow()
            aa = ApproximateWindow()
            aa.setUi(self.win, name="图像", paths=pathss)
            self.win.show()
